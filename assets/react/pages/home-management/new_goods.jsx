...
class RecommendGoods extends React.Component {
    ...
    //初始化
    init(){
        H.server.indexManage_newGoods_list(this.state.defaultPara, (res)=>{
            if(res.code == 0){
                this.setState({
    ...
    bodyOperate(e){
        ...
        switch (text){
            case '下移':
                ...
                H.server.indexManage_newGoods_move({
                    current_id: id,
            ...
            case '下架':
                H.Modal({
                    ...
                    cancel: true,
                    okCallback: () => {
                        H.server.indexManage_newGoods_pullOff({
                            id: id
                        }, (res)=>{
                            if(res.code == 0){
                                H.Modal('下架成功');
    ...
    add(){
        ...
        H.server.indexManage_newGoods_add(para, (res)=>{
            if(res.code == 0){
                H.Modal('添加成功');
            }else if(res.code == 10106) {
                H.overdue();
